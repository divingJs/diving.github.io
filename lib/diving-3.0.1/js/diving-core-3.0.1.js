(function () {
    ...
    diving.fn = diving.prototype = {
        ...
        show: function(){
            this[0].style.display=this[0].oldblock?this[0].oldblock:'';
            if( diving.css(this[0],'display')=='none')
                this[0].style.display='block';
        },
        hide:function(){
            this[0].oldblock=this[0].oldblck||diving.css(this[0],'display');
            if(this[0].oldblock=='none')
                this[0].oldblock='block';
        ...
        hasClass:function(c){
            var elm=(this.hasOwnProperty('classList'))?this:this[0];
            return diving.className.has(elm,c);
        },
        remove:function(a){
...
diving.extend({
    ...
    diving.event = {
        ...
        trigger: function(event, data, elem, onlyHandlers) {
            data = diving.makeArray(data||[]);
            if(!elem){
                var g = this.global[event];
                if(g){
                    diving.each(g,function(){
                        diving.event.trigger(event,data,this);
                    });
                }
            }else if( elem['on'+event]){
                data.unshift(this.fix({type:event,target:elem}));
                var val = elem['on'+event].apply(elem,data);
                //if(val!==false&&diving.isFunction(elem[event]))elem[event]();
            }
        },
        ...
        fix: function(event) {
            ...
            if (diving.browser.safari && event.target.nodeType == 3) {
                var originalEvent = event;
                event = diving.extend({}, originalEvent);
    ...
    diving.extend({
        ...
        css: function(e,p) {
            ...
            if ( p == "height" || p == "width" ) {
                ...
                diving.each( d, function(){
                    old["padding" + this] = 0;
                    old["border" + this + "Width"] = 0;
                });

                diving.swap( e, old, function() {
                    if (diving.css(e,"display") != "none") {
                        oHeight = e.offsetHeight;
                        oWidth = e.offsetWidth;
                    } else {
                        e = diving(e.cloneNode(true))
                            .find(":radio").removeAttr("checked").end()
                            .css({
                                visibility: "hidden", position: "absolute", display: "block", right: "0", left: "0"
                            }).appendTo(e.parentNode)[0];

                        var parPos = diving.css(e.parentNode,"position");
            ...
            return diving.curCSS( e, p );
        },

        curCSS: function(elem, prop, force) {
            var ret;
            
            if (prop == "opacity" && diving.browser.msie)
                return diving.attr(elem.style, "opacity");
                
            if (prop == "float" || prop == "cssFloat")
                prop = diving.browser.msie ? "styleFloat" : "cssFloat";
            ...
            if (!force && elem.style[prop])
                ...
                if ( cur )
                    ...
                    diving.swap(elem, { display: "block" }, function() {
